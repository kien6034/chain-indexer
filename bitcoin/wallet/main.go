...
func CreateTx(privKey string, destination string, amount int64) (string, error) {
	...
	// Generate a SegWit (bech32) address using the testnet parameters
	addrWitnessPubKey, err := btcutil.NewAddressWitnessPubKeyHash(btcutil.Hash160(wif.PrivKey.PubKey().SerializeCompressed()), &chaincfg.TestNet3Params)
	if err != nil {
		return "", err
	}

	fmt.Println("address is: ", addrWitnessPubKey.EncodeAddress())
	return "nil", nil
